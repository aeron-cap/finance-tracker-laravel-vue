    ...
    <div class="min-h-screen pb-20">
        <div class="h-full px-4 py-6 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                <div class="mb-8">
                    <h1 class="text-4xl sm:text-5xl font-bold text-white">Expenses</h1>
                ...
                <div v-else class="relative">
                    <div v-if="expenses_data.length === 0" class="text-center py-16">
                        ...
                        <h3 class="text-2xl font-bold text-white mb-2">No expense records found</h3>
                        <p class="text-gray-400 mb-6">Start by adding your first expense</p>
                    </div>
                    ...
                    <div v-else class="mt-6 mb-6">
                        <div class="hidden md:block relative overflow-x-auto rounded-xl border border-gray-700/50">
                            <table class="w-full text-sm text-left text-gray-400">
                                ...
                                <tbody>
                                    <tr v-for="(expense, index) in expenses_data.slice(from, to)" :key="index" class="bg-slate-800/30 border-b border-gray-700/50 hover:bg-slate-700/30 transition-colors">
                                        ...
                                        <td class="px-6 py-4 font-semibold text-red-400 text-lg">
                                            {{ $formatToDecimal(expense.amount) }}
                                        </td>
                                        <td class="px-6 py-4">
                                            <button @click="edit_record(expense.id)" class="font-medium text-blue-400 hover:text-blue-300 transition-colors">
                                                Edit
                                            </button>
                                        </td>
                                    </tr>
                            ...
                            <nav class="flex items-center flex-wrap justify-between p-4 bg-slate-700/30" aria-label="Table navigation">
                                <span class="text-sm font-normal text-gray-400 mb-4 md:mb-0">
                                    Showing 
                                    <span class="font-semibold text-white">{{ from + 1 }}-{{ Math.min(to, expenses_data.length) }}</span> 
                                    of 
                            ...
                            >
                                ...
                                <div class="space-y-2">
                                    <p class="text-sm text-gray-300">
                                        <span class="text-gray-500">Account:</span> {{ expense.account_name }}
                                    </p>
                                    <p class="text-sm text-gray-300">
                                        <span class="text-gray-500">Description:</span> {{ expense.description }}
                                    </p>
                                </div>
